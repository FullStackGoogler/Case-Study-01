...
def find_best_weights(title_similarities, summary_similarities, category_similarities, genre_similarities, tags_similarities, team_similarities, games_filtered):
    ...
    for title_weight, summary_weight, categories_weight, genres_weight, tags_weight, team_weight in product(weights_range, repeat=6):
        if np.isclose(title_weight + summary_weight + categories_weight + genres_weight + tags_weight + team_weight, 1.0):
            ...
            if avg_similarity > best_avg_similarity:
                ...
                best_avg_weights = (title_weight, summary_weight, categories_weight, genres_weight, tags_weight, team_weight)
            ...
            if max_similarity > best_max_similarity:
                ...
                best_max_weights = (title_weight, summary_weight, categories_weight, genres_weight, tags_weight, team_weight)
...
def calculate_similarities(name, data_original, data_filtered, use_local_model):
    ...
    selected_game_category = str(result.Categories.item())
    selected_game_genres = str(result.Genres.item())
    selected_game_tags = str(result.Tags.item())
    ...
    all_game_categories = games_filtered['Categories'].apply(lambda x: ' '.join(sorted(str(x)))).tolist()
    all_game_genres = games_filtered['Genres'].apply(lambda x: ' '.join(sorted(str(x)))).tolist()
    all_game_tags = games_filtered['Tags'].apply(lambda x: ' '.join(sorted(str(x)))).tolist()
    
    all_game_teams = games_filtered['Team'].apply(lambda x: ' '.join(sorted(str(x)))).tolist()

    # Ensure all elements are strings
    all_game_titles = [str(title) for title in all_game_titles]
    summaries_all_games = [str(summary) for summary in summaries_all_games]
    all_game_categories = [str(cateogory) for cateogory in summaries_all_games]
    all_game_genres = [str(genre) for genre in summaries_all_games]
    all_game_tags = [str(tag) for tag in summaries_all_games]
    ...
    if use_local_model:
        ...
        # Get similarities for categories
        category_similarities = client.sentence_similarity(
            selected_game_category,
            other_sentences=all_game_categories,
            model='sentence-transformers/all-MiniLM-L6-v2'
        )

        # Get similarities for genres
        genre_similarities = client.sentence_similarity(
            selected_game_genres,
            other_sentences=all_game_genres,
            model='sentence-transformers/all-MiniLM-L6-v2'
        )

        # Get similarities for tags
        tags_similarities = client.sentence_similarity(
            selected_game_tags,
            other_sentences=all_game_tags,
            model='sentence-transformers/all-MiniLM-L6-v2'
        )
        ...
        best_avg_weights, best_max_weights, best_avg_similarity, best_max_similarity = find_best_weights(
            title_similarities, summary_similarities, category_similarities, genre_similarities, tags_similarities, team_similarities, games_filtered
        )
